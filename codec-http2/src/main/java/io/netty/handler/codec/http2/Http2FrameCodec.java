 ...
 *
 * The frame demo delivers and writes frames for active streams. An active stream is closed when either side sends a
 ...
 * <h3>Flow control</h3>
 *
 * The frame demo automatically increments stream and connection flow control windows.
 *
 * <p>Incoming flow controlled frames need to be consumed by writing a {@link Http2WindowUpdateFrame} with the consumed
 * number of bytes and the corresponding stream identifier set to the frame demo.
 *
 ...
 * reference counted objects (e.g. {@link ByteBuf}s). The frame demo will call {@link ReferenceCounted#retain()} before
